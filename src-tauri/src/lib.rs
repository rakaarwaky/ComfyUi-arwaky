  ...
  if let Some(sender) = app_handle.try_state::<LogSender>() {
    if let Ok(tx_guard) = sender.tx.lock() {
      if let Some(ref tx) = *tx_guard {
        if tx.try_send(LogMessage::Launcher(message.to_string())).is_err() {
...
#[tauri::command]
fn check_backend_status(backend: tauri::State<'_, BackendState>) -> (bool, Option<String>) {
...
fn start_backend_download(
  ...
  {
    let mut guard = backend.cancel_token.lock()
  ...
  std::thread::spawn(move || {
    ...
    let result = installer.install(|progress| {
...
fn detect_dgpu_index() -> String {
  ...
  match output {
          ...
          if let (Some(start_idx), Some(end_idx)) = (line.find('['), line.find(']')) {
            ...
            if let Some(col_idx) = line.rfind(':') {
              ...
              if let Ok(vram_bytes) = vram_str.parse::<u64>() {
                eprintln!("[GPU Detection] GPU {}: {} bytes VRAM", gpu_num, vram_bytes);
                if vram_bytes > max_vram {
                  max_vram = vram_bytes;
                  best_gpu = gpu_num.to_string();
                }
              } else {
                eprintln!("[GPU Detection] Failed to parse VRAM value: '{}'", vram_str);
              }
            }
          }
        }
    ...
    Ok(out) => {
      eprintln!("rocm-smi exited with error: {}", String::from_utf8_lossy(&out.stderr));
      "0".to_string()
    }
    Err(e) => {
      ...
      for entry in entries.flatten() {
        ...
        if let Ok(content) = std::fs::read_to_string(&gfx_path) {
          ...
          if let Ok(version) = ver_str.parse::<u32>() {
            ...
            eprintln!("[GPU Detection] sysfs node {}: gfx_target_version={} → gfx{}.{}.{}",
              entry.file_name().to_string_lossy(), version, major, _minor, patch);
...
fn spawn_comfyui_process(app_handle: &tauri::AppHandle) -> Result<(), String> {
  ...
  let mut extra_model_paths = current_dir.join("extra_model_paths.yaml");
  if !extra_model_paths.exists() {
    if let Ok(res_dir) = app_handle.path().resource_dir() {
      let test_path = res_dir.join("extra_model_paths.yaml");
      if test_path.exists() {
        extra_model_paths = test_path;
      }
    }
  }
  if !extra_model_paths.exists() {
  ...
  log_info(app_handle, &format!("Smart GPU Detection: Using GPU {} (discrete GPU with largest VRAM)", gpu_index));
  if let Some(ver) = hsa_override {
    log_info(app_handle, &format!("HSA_OVERRIDE_GFX_VERSION={} (GPU variant requires override)", ver));
  }
  ...
  let mut child = match cmd.spawn() {
    Ok(child) => {
      log_info(app_handle, &format!("Python process spawned with PID: {}", child.id()));
      child
    }
    Err(err) => {
  ...
  let stderr_handle = std::thread::spawn(move || {
    ...
    for line in reader.lines() {
      stats.total_received.fetch_add(1, Ordering::Relaxed);
      if let Ok(line_str) = line {
        if log_tx_stderr.try_send(LogMessage::Stderr(line_str)).is_err() {
          stats.dropped.fetch_add(1, Ordering::Relaxed);
        }
      }
    }
  ...
  let polling_handle = std::thread::spawn(move || {
    ...
    log_info(&app_handle_poll, &format!("Starting port check on {}...", port_addr));
    ...
    loop {
...

      {
        ...
        if let Some(ref mut child) = *lock {
          match child.try_wait() {
            Ok(Some(status)) => {
              let err_msg = format!("Python process exited unexpectedly with status: {:?}", status);
            ...
            Ok(None) => {}
            Err(e) => {
              log_info(&app_handle_poll, &format!("Failed to check child process status: {:?}", e));
            }
          }
        }
      }

      if std::net::TcpStream::connect_timeout(&socket_addr, std::time::Duration::from_millis(PORT_CONNECT_TIMEOUT_MS)).is_ok() {
        log_info(&app_handle_poll, &format!("ComfyUI server responsive on {}. Redirecting window...", port_addr));
        ...
        if let Some(window) = app_handle_poll.get_webview_window("main") {
          if let Ok(url) = "http://127.0.0.1:8188".parse::<tauri::Url>() {
            ...
            if let Err(e) = window.navigate(url) {
              log_info(&app_handle_poll, &format!("Error navigating webview window: {:?}", e));
            }
          } else {
      ...
      if count >= PORT_POLL_TIMEOUT_SECS && !timeout_emitted {
        let timeout_msg = format!("Failed to connect to ComfyUI after {} seconds. Please check the logs below.", PORT_POLL_TIMEOUT_SECS);
        ...
        log_info(&app_handle_poll, &format!("Still waiting... {} seconds elapsed.", count));
      }
...
pub fn run() {
  ...
  let app = tauri::Builder::default()
    ...
    .setup(|app| {
      ...
      let writer_handle = std::thread::spawn(move || {
        ...
        loop {
          match log_rx.recv_timeout(Duration::from_millis(BATCH_FLUSH_INTERVAL_MS)) {
            Ok(msg) => {
              let is_redirected = redirect_state.is_redirected.load(Ordering::Acquire);
      ...
      // Check backend status and emit start event
