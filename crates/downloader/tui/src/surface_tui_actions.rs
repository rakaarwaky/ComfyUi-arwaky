...
impl App {
    ...
    pub fn copy_logs_to_clipboard(&self) -> String {
        ...
        let p = SESSION_LOG_PATH
            ...
            .unwrap_or_default();
        if ok {
            format!("✓ Logs copied ({} lines)", self.logs.len())
        } else {
            format!("Log: {p} ({} lines)", self.logs.len())
        }
    }
}
...
impl App {
    pub fn save_config_to_file(&self) -> std::io::Result<()> {
        ...
        if let Ok(h) = std::env::var("HOME") {
            let _ = fs::write(
                PathBuf::from(&h).join(".config/comfyui-downloader/config.yaml"),
                &c,
            );
        }
        Ok(())
...
impl App {
    pub fn check_space_and_start(&mut self) {
        ...
        if self.selected_indices.is_empty() {
            self.add_log("No models selected.");
            return;
        }
        let mut need = 0u64;
        for &i in &self.selected_indices {
            ...
            let d = self
                ...
                .join(&m.filename);
            if !file_exists_valid(&d, m.size_bytes, Some(&m.url)) {
                need += m.size_bytes;
            }
        }
        let avail =
            get_available_space(&PathBuf::from(&self.config.models_dir)).unwrap_or(u64::MAX);
        if avail < need {
    ...
    fn start_download_via_orch(&mut self) {
        ...
        let selected: Vec<(usize, Model)> = self
            .selected_indices
            .iter()
            .map(|&i| (i, self.models[i].clone()))
            .collect();
    ...
    pub fn refresh_sizes(&mut self) {
        ...
        let models: Vec<(usize, Model)> = indices
            .iter()
            .filter_map(|&i| self.models.get(i).map(|m| (i, m.clone())))
            .collect();
        if models.is_empty() {
            return;
        }
            ...
            match e {
                ...
                DownloadEvent::Progress {
                    ...
                    if let Some(ref mut a) = self
                        ...
                        .and_then(|o| o.as_mut())
                    {
                        a.bytes_downloaded = downloaded;
                        a.total_bytes = total;
                        a.speed_mb_s = speed_mb_s;
                        a.eta_secs = eta_secs;
                    }
                }
                ...
                DownloadEvent::AllComplete { completed, failed } => {
                    ...
                    self.add_log(&format!(
                        "Queue done. OK: {completed}, Fail: {failed} (of {total})"
                    ));
                    self.state = AppState::Finished {
                        completed,
                        failed,
                        message: format!("Done! OK: {completed}, Fail: {failed}"),
                    };
                ...
                DownloadEvent::RefreshUpdate { idx, size } => {
                    if let Some(m) = self.models.get_mut(idx) {
                        m.size_bytes = size;
                        if let Ok(mut c) = SIZE_CACHE.write() {
                            c.sizes.insert(m.url.clone(), size);
                        }
                    }
                }
                DownloadEvent::RefreshFinished { .. } => {
                    if let Ok(c) = SIZE_CACHE.write() {
                        c.save();
                    }
                    clear = true;
                    self.mark_filtered_dirty();
