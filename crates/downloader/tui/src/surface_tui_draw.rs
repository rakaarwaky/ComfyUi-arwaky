...
pub fn draw_ui(f: &mut ratatui::Frame, app: &mut App) {
    ...
    let outer_block = Block::default()
        .title(format!(
            " ComfyUI Desktop Model Downloader v2.2 (Ratatui TUI){title_suffix} "
        ))
        .borders(Borders::ALL)
    ...
    let items: Vec<ListItem> = filtered
        ...
        .map(|(orig_idx, m)| {
            ...
            let size = if m.size_bytes > 0 {
                m.size_bytes
            } else if let Ok(cache) = SIZE_CACHE.read() {
                cache.sizes.get(&m.url).copied().unwrap_or(0)
            } else {
                0
            };
    ...
    let help_text = if app.input_mode == InputMode::Search {
        ...
        "  [Tab/Shift+Tab] Cycle Tabs  |  [Space] Toggle  |  [a] Select All Missing  |  [x] Cancel Download  |  [c] Settings  |  [COPY LOGS]  "
    } else {
        "  [Tab/Shift+Tab] Cycle Tabs  |  [Space] Toggle  |  [a] Select All Missing  |  [r] Refresh  |  [c] Settings  |  [Enter/d] Download  |  [COPY LOGS]  "
    };
    ...
    match app.state.clone() {
        ...
        AppState::Settings {
            ...
            let settings_spans = vec![
                ...
                Line::from(""),
                Line::from(Span::styled(
                    "Use [Tab] or [Up/Down] to navigate. Type to edit path/token.",
        ...
        AppState::Finished { completed, failed, ref message } => {
            ...
            let finished_spans = vec![
                ...
                Line::from(vec![
                    Span::raw("   "),
                    Span::styled("  [ OK ]  ", Style::default().fg(Color::Cyan)),
                ]),
            ];
