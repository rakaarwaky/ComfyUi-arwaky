            ...
            if let Some(url_str) = url {
                if let Ok(cache) = crate::SIZE_CACHE.read() {
                    if let Some(&cached_size) = cache.sizes.get(url_str) {
                        if is_size_valid(cached_size) && expected_size == 0 { return true; }
                    }
                }
            }
            if expected_size == 0 { return actual_size >= 1000; }
            return is_size_valid(expected_size);
        }
    } else if path.is_dir() {
        if let Ok(entries) = fs::read_dir(path) { return entries.count() > 5; }
    }
    false
}
...
pub fn sanitize_filename(filename: &str) -> String {
    filename.chars().filter(|&c| c.is_alphanumeric() || c == '.' || c == '_' || c == '-')
...
mod tests {
    ...
    fn sanitize_removes_special_chars() {
        // Parentheses removed, leaving contiguous chars
        assert_eq!(
    ...
    fn sanitize_triple_dot_becomes_double_dot() {
        // "..." → replace ".." with "." → ".." → stays (only one pass)
        assert_eq!(sanitize_filename("test...safetensors"), "test..safetensors");
    }
    ...
    fn sanitize_replaces_path_separators() {
        // Forward slashes should be removed, resulting in concatenation
...
pub fn get_available_space(path: &Path) -> std::io::Result<u64> {
    ...
    while !check_path.exists() { if let Some(parent) = check_path.parent() { check_path = parent.to_path_buf(); } else { break; } }
    let mut stats = std::mem::MaybeUninit::<libc::statvfs>::uninit();
    let c_path = CString::new(check_path.as_os_str().as_bytes()).map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidInput, e))?;
    ...
    if res == 0 { let stats = unsafe { stats.assume_init() }; Ok(stats.f_bavail * stats.f_frsize) }
