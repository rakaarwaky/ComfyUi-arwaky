...
use launcher_shared::{normalize_path, ArchiveUrl, BackendInstallError, BackendInstallEvent, InstallDir, Sha256Hash};
...
impl BackendInstallProtocol for BackendInstaller {
    ...
    fn expected_sha256(&self) -> Option<Sha256Hash> {
        Some(Sha256Hash("e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855".to_string()))
    }
    ...
    fn install(
        ...
        BackendInstaller::download_archive(&archive_path, archive_url.as_str(), on_event, cancel_token)
            .map_err(BackendInstallError::ConnectionFailed)?;

        if let Some(expected) = expected_sha256 {
            on_event(BackendInstallEvent::Verifying);
            BackendInstaller::verify_checksum(&archive_path, expected.as_str())
                .map_err(|e| BackendInstallError::ChecksumMismatch {
        ...
        BackendInstaller::extract_archive(install_dir.as_path(), &archive_path, on_event, cancel_token)
